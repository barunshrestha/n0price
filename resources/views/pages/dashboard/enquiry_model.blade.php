        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap border-1">
                <div class="card-title">
                    <h3 class="card-label">
                        New Enquiries by Model
                    </h3>
                </div>
                <div class="card-toolbar">
                    <a class="btn btn-icon btn-sm btn-hover-light-primary mr-1" data-toggle="collapse"
                        href="#enquiry-model" role="button" aria-expanded="true" aria-controls="custom_kt"
                        title="Toggle Card">
                        <i class="ki ki-arrow-down icon-nm"></i>
                    </a>
                </div>
            </div>
            <div id="enquiry-model" class="collapse show">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php
                            if (isset($enquiries_by_model) && !empty($enquiries_by_model)) {
                                $ctn=1; foreach ($enquiries_by_model as $name => $data):
                                ...
                                <tr>
                                    ...
                                    for ($i = 0; $i < count($date_range); $i++) {
                                        $total_enquiries = $data[$date_range[$i]]['total_enquiries'] != 0 ? $data[$date_range[$i]]['total_enquiries'] : '';
                            ...
                            <?php if(isset($total_enquiries_model_by_date) && !empty($total_enquiries_model_by_date)) { ?>
                            <tfoot>
                                <tr>
                                    <td colspan="2" style="font-size: 14px;font-weight: 600;">Total Enquiries</td>
                                    <?php
                                    for ($i = 0; $i < count($date_range); $i++) {
                                        echo '<td><label style="color: green; font-weight:bold;">' . $total_enquiries_model_by_date[$date_range[$i]]['total_enquiries'] . '</label></td>';
                                    }
                                    ?>
                                </tr>
                            </tfoot>
