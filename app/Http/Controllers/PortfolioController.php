...
class PortfolioController extends Controller
{
    ...
    {
        $portfolio = Portfolio::where('user_id', Auth::id())->get(['id', 'portfolio_name', 'status']);
        $this->_data['portfolios'] = $portfolio;
        $user = Auth::user();
        $selected_portfolio = Portfolio::where('status', 1)->where('user_id', $user->id)->get(['id', 'portfolio_name', 'status']);
    ...
    {
        $user = Auth::user();
        Portfolio::where('status', 1)->where('user_id', $user->id)->update(["status" => 0]);
        Portfolio::where('user_id', $user->id)->where('id', $id)->update(["status" => 1]);
